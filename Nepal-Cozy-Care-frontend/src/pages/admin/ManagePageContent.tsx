...
const INITIAL_PAGES: PageItem[] = [
  { key: "navigation_menu", name: "Navbar & Dropdowns", url: "/plants", payload: {}, updated_at: null },
...
function NavigationMenuEditor({
  ...
  const renderDropdownList = (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
      ...
      <button
        type="button"
        className="admin-btn admin-btn-secondary"
        onClick={() => addItem(groupKey, { ...defaultNewItem, id: `item_${Date.now()}` })}
        style={{
          marginTop: "0.5rem",
          display: "inline-flex",
          alignItems: "center",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
      {/* Plants dropdown */}
      <EditorCard
        title="🌿 Plants Dropdown Menu (Image 1)"
        description="Shown when hovering over the 'Plants' navbar item. Clicking 'Plants' navigates to /plants to display all plants."
        badge={`${plantsDropdown.length} Items`}
      >
        {renderDropdownList("plants_dropdown", plantsDropdown, {
          label: "New Plant Category",
          path: "/plants?type=custom",
          is_active: true,
        })}
      </EditorCard>

      {/* Shop by Location nested flyout */}
      <EditorCard
        title="🏡 Shop by Location Sub-Menu / Flyout (Image 2)"
        description="Nested flyout menu opening to the right of 'Shop by Location'. Includes Bedroom, Living Room, Kitchen, Bathroom, Balcony, Workspace."
        badge={`${locationDropdown.length} Rooms`}
      >
        {renderDropdownList("location_dropdown", locationDropdown, {
          label: "New Room / Location",
          path: "/plants?location=room",
          is_active: true,
        })}
      </EditorCard>
      ...
      {/* Care Tips dropdown */}
      <EditorCard
        title="🌱 Care Tips Dropdown Menu (Image 3)"
        description="Shown when hovering over or viewing the 'Care Tips' navigation item. Links to potting mixes, tools, accessories, pest control, or Doctor Green."
        badge={`${careTipsDropdown.length} Items`}
      >
        {renderDropdownList("care_tips_dropdown", careTipsDropdown, {
          label: "New Care Option",
          path: "/pots?category=care",
          is_active: true,
        })}
      </EditorCard>
      ...
      <EditorCard
        title="🏺 Accessories Dropdown Menu"
        description="Shown when hovering over the 'Accessories' navigation link. Clicking 'Accessories' directly navigates to /pots to display all accessories."
